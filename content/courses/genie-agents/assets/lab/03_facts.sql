...
-- ============================================================================
CREATE OR REPLACE TABLE mfg.core.fct_transactions
...
AS
WITH t AS (
  SELECT id AS n FROM range(1, 20000001)   -- << TIER >>
),
calc AS (
  SELECT
    n,
...
SELECT
  ...
  s.status
FROM shaped s
JOIN mfg.core.dim_account  a ON a.account_id  = s.account_id
JOIN mfg.core.dim_customer c ON c.customer_id = a.customer_id;

ALTER TABLE mfg.core.fct_transactions ALTER COLUMN fee_revenue
...
ALTER TABLE mfg.core.fct_transactions ALTER COLUMN status
...
ALTER TABLE mfg.core.fct_transactions ALTER COLUMN currency
...
ALTER TABLE mfg.core.fct_transactions ALTER COLUMN amount
  COMMENT 'Transaction amount in the settlement currency (see currency column), not USD.';
ALTER TABLE mfg.core.fct_transactions ALTER COLUMN merchant_category
...
-- ============================================================================
CREATE OR REPLACE TABLE mfg.core.fct_reversals
...
FROM mfg.core.fct_transactions t
...
ALTER TABLE mfg.core.fct_reversals ALTER COLUMN reversal_amount
  COMMENT 'Fee revenue clawed back. Subtract the sum of this from gross fee_revenue to get NET fee revenue.';
ALTER TABLE mfg.core.fct_reversals ALTER COLUMN reason_code
...
-- ============================================================================
CREATE OR REPLACE TABLE mfg.core.fct_loan_balances
...
WITH loan_accounts AS (
  ...
  FROM mfg.core.dim_account a
  JOIN mfg.core.dim_product p ON p.product_id = a.product_id
...
ALTER TABLE mfg.core.fct_loan_balances ALTER COLUMN principal_balance
...
ALTER TABLE mfg.core.fct_loan_balances ALTER COLUMN days_past_due
  COMMENT 'Days past due as of snapshot_date. 0 means current.';
ALTER TABLE mfg.core.fct_loan_balances ALTER COLUMN dpd_bucket
...
ALTER TABLE mfg.core.fct_loan_balances ALTER COLUMN loan_status
...
-- ============================================================================
CREATE OR REPLACE TABLE mfg.core.fct_applications
...
ALTER TABLE mfg.core.fct_applications ALTER COLUMN decision
...
ALTER TABLE mfg.core.fct_applications ALTER COLUMN funded_ts
...
ALTER TABLE mfg.core.fct_applications ALTER COLUMN channel
...
-- fct_fraud_cases
-- ============================================================================
CREATE OR REPLACE TABLE mfg.core.fct_fraud_cases
...
ALTER TABLE mfg.core.fct_fraud_cases ALTER COLUMN fraud_type
  COMMENT 'Fraud typology. Values: CARD_NOT_PRESENT, ACCOUNT_TAKEOVER, APPLICATION_FRAUD, CHECK_FRAUD, WIRE_FRAUD, FIRST_PARTY.';
ALTER TABLE mfg.core.fct_fraud_cases ALTER COLUMN loss_amount
...
-- Row counts
-- ----------------------------------------------------------------------------
SELECT 'fct_transactions'  AS table_name, count(*) AS rows FROM mfg.core.fct_transactions
UNION ALL SELECT 'fct_reversals',      count(*) FROM mfg.core.fct_reversals
UNION ALL SELECT 'fct_loan_balances',  count(*) FROM mfg.core.fct_loan_balances
UNION ALL SELECT 'fct_applications',   count(*) FROM mfg.core.fct_applications
UNION ALL SELECT 'fct_fraud_cases',    count(*) FROM mfg.core.fct_fraud_cases
ORDER BY table_name;
