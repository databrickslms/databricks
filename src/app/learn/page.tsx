...
import { TRACKS, byStage, modulesForTrack } from '@/lib/content'
...
export default async function LearnHome() {
  ...
  return (
    <div className="max-w-4xl">
      ...
      {next && (
        <Link href={`/learn/${next.slug}`} className="card card-hover mt-8 block p-5">
      ...
      <section className="mt-12">
        ...
        <div className="mt-5 space-y-8">
          {byStage(modules).map(({ stage, modules: mods }) => (
            <div key={stage}>
              ...
              <ul className="grid gap-2 sm:grid-cols-2">
                {mods.map((m) => {
                  ...
                  return (
                    <li key={m.slug}>
                      <Link href={`/learn/${m.slug}`} className="card card-hover flex h-full items-start gap-3 p-3.5">
