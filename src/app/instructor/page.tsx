...
import { TRACKS, modulesForTrack, type Track } from '@/lib/content'
import { getCohort } from '@/lib/progress'
...
export default async function InstructorPage() {
  ...
  const trackTotal = (t: string | null) =>
    modulesForTrack((t ?? 'author') as Track).length

  const active = cohort.filter((r) => Number(r.completed) > 0 || Number(r.quizzesTaken) > 0)
  const avgAll = active.length
  ...
  return (
    <div className="mx-auto max-w-[1200px] px-5 py-12">
      ...
      <p className="mt-2 text-[0.9rem] text-muted">
        Everyone who has signed in, with completion and knowledge-check scores.
      </p>

      <div className="mt-8 grid gap-3 sm:grid-cols-3">
        {[
          ['Learners', String(cohort.length)],
          ['Active', String(active.length)],
          ['Avg quiz score', avgAll === null ? '—' : `${avgAll}%`],
        ].map(([label, value]) => (
      ...
      {cohort.length === 0 ? (
        <p className="mt-10 text-[0.9rem] text-muted">
          No learners yet. Share the site and ask people to sign in with Google.
        </p>
      ) : (
        <div className="table-scroll mt-8">
          <table className="w-full text-[0.855rem]">
            ...
            <tbody>
              {cohort.map((r) => {
                const total = trackTotal(r.track)
                ...
                return (
                  <tr key={r.id} className="border-b last:border-0">
                    ...
                    <td className="whitespace-nowrap px-3 py-2.5 text-muted">
                      {TRACKS[(r.track ?? 'author') as Track]?.name ?? r.track}
                    </td>
                    <td className="px-3 py-2.5">
                      <div className="flex items-center gap-2">
                        <div className="h-1.5 w-16 overflow-hidden rounded-full" style={{ background: 'rgb(var(--line))' }}>
                          <div
