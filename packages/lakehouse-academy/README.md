...
The SQL is **authored** in `content/courses/<id>/assets/lab/` so the LMS and this
package read the same files. Sync the package's copy with:

```bash
python sync_sql.py            # copy
python sync_sql.py --check    # fail on drift, for CI
```

Placeholders available in the SQL: `{{CATALOG}}`, plus anything declared under a
tier's `values` (currently `{{TXN_COUNT}}`). An unresolved placeholder raises
